...
tools:
  ...
  maxTotalSubCalls: 0
  maxTotalDeliveredValueBytes: 0
```

`maxParallelSubCalls` bounds only the overlapping Code Mode dispatch window; `maxTotalSubCalls` bounds accepted sub-dispatch submissions across one `run_code`, including calls that are still queued. The total cap defaults to `0` (disabled) until workload evidence supports a product threshold. These are independent controls: lowering parallelism does not reduce the amount of work a program may submit. Successful durable `tool/code-dispatch` settles also record exact JSON UTF-8 bytes of the canonical value delivered to the program; `maxTotalDeliveredValueBytes` can use that same canonical byte definition to reject only the delivery of a successful nested result when the per-run cumulative cap would be exceeded, while preserving the underlying tool outcome as success; execution accounting reports legacy successful settles without that evidence separately instead of treating unknown bytes as zero. The behavior-neutral projection is exported as `@deepseek-ai/dsh-tools/execution-accounting` for Agent DevTools and other evidence consumers; it remains unsuitable as a live admission counter.
