...
`maxParallelSubCalls` 只限制 Code Mode 同时重叠的 dispatch 窗口；`maxTotalSubCalls` 则限制一次 `run_code` 接受的 sub-dispatch 提交总数，仍在队列中的调用也立即计数。总量上限默认 `0`（禁用），直到代表性 workload evidence 足以支持产品阈值。两者彼此独立：降低并发不会自动降低程序可提交的总工作量。成功的持久化 `tool/code-dispatch` settle 还会记录实际交付给程序的规范值所占的精确 JSON UTF-8 字节数；`maxTotalDeliveredValueBytes` 使用同一规范字节定义，在单次运行累计上限将被突破时只拒绝成功嵌套结果的交付，同时保留底层工具执行为成功；execution accounting 会把缺少该证据的旧版成功 settle 单独计数，而不会把未知字节误当成 0。行为中性的 projection 以 `@deepseek-ai/dsh-tools/execution-accounting` 导出，供 Agent DevTools 等证据消费者使用；它仍不能作为实时 admission 计数器。
