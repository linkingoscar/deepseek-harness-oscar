...
`maxParallelSubCalls` 只限制 Code Mode 同时重叠的 dispatch 窗口；`maxTotalSubCalls` 则限制一次 `run_code` 接受的 sub-dispatch 提交总数，仍在队列中的调用也立即计数。总量上限默认 `0`（禁用），直到代表性 workload evidence 足以支持产品阈值。两者彼此独立：降低并发不会自动降低程序可提交的总工作量。
