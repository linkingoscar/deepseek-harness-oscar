...
Every field is validated and defaulted. `maxOutputBytes` is a safe integer of at least four bytes; `maxBindingValueBytes` is a non-negative safe integer whose zero default keeps the new admission mechanism disabled until representative benchmark evidence justifies a product threshold. The remaining fields are positive finite numbers, and `maxWallMs` is additionally at most `2147483647` (Node's maximum `setTimeout` delay).
...
- **Time budgets are independent because the peer is hostile** — `computeMs` meters the worker's MEASURED busy time (`worker.performance.eventLoopUtilization()` polling): a hot loop cannot hide behind a pending decoy dispatch, and a program awaiting a slow tool accrues nothing. `maxWallMs` backstops what busy time cannot see (awaiting a promise nobody resolves). Both funnel into `worker.terminate()`, which ends hot synchronous loops too; heap overflow surfaces as the worker's OOM exit (`kind: 'worker-exit'`). `maxWallMs` is range-checked at load against `MAX_TIMER_DELAY_MS`: `setTimeout` clamps a longer delay to 1 ms, so a positivity check alone would accept a ceiling that expires on the first tick. `computeMs` needs no such bound, being compared against measured utilization rather than fed to a timer.
- **Intermediate binding values are complete JSON with optional per-value admission** — binding arguments and resolutions undergo iterative lossless-JSON validation. Before program execution, the worker captures its own realm's plain-container prototype identities plus the native function-source check used only for foreign realms, so constructor-slot mutation and user-authored impostors cannot change container classification. It also captures every structural and metering intrinsic used by this JSON boundary, creates property descriptors without a prototype, and bypasses mutable collection prototypes for private traversal state; model mutations of globals, prototype methods, or descriptor-shaped `Object.prototype` fields therefore cannot alter validation, wire transport, or byte accounting. Values flatten into a bounded-depth pre-order wire value for structured clone and rebuild iteratively on the other side. When `maxBindingValueBytes > 0`, exact serialized JSON bytes are admitted at every complete-value boundary: the worker checks an argument before allocating/posting the call, the hostile host checks decoded arguments again before invoking the binding, and the host checks a snapshotted resolution before posting it back. Each argument or resolution is admitted independently; binding traffic never enters the outer-output ledger or model context.
...
- **Binding admission is per complete value, not cumulative execution accounting** — `maxBindingValueBytes` does not cap total bytes acquired across a run, concurrent dispatches, or total dispatch count. Its default remains disabled (`0`) until representative benchmark workloads support a public threshold; cumulative admission belongs to the broader Execution Plane budget work.
