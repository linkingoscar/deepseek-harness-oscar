...
`@deepseek-ai/dsh-session-query/evidence-inspector` exports `readSessionEvidence(sessionId, events, options?)`, a pure developer projection over an already obtained ordered event log. One selected prefix feeds the existing Code Mode execution accounting/diagnostics and replay capability inspector, so the returned `session`, `execution`, and `replay` sections describe the same boundary. `execution` keeps the existing `maxRunPeakInFlight` name and byte-overflow `null` semantics instead of claiming global concurrency or inventing a measurable byte total. `replay.modes` reuses the existing request-reconstruction, simulated, live-fork, and reproducible capability records; evidence and snapshot fields report presence only after the replay inspector's existing validation and fail-closed selection. The projection appends no session event, persists no derived record, and defines no diagnostics metric, replay blocker, replay mode, or evidence format.

`options.boundary` is the inclusive event sequence passed to replay inspection. `options.sourceKind` records how the caller obtained the log as `live`, `persisted`, or `supplied-log`; direct inspection defaults to `supplied-log` because an event stream alone cannot prove whether it came from a live or persisted source.
